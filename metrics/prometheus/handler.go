...
// Prometheus text exposition format without adding a client dependency.
package prometheus
...
func (h *Handler) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	var buffer [ridstore.MetricSampleCount]ridstore.MetricSample
	samples := h.source.Metrics().AppendMetricSamples(buffer[:0])
	...
	for _, sample := range samples {
		...
		fmt.Fprintf(&body, "# TYPE %s %s\n%s%s %s\n", sample.Name, kind, sample.Name, h.labels, strconv.FormatUint(sample.Value, 10))
	}
...
func validLabelName(name string) bool {
	if name == "" || !labelCharacter(name[0]) {
		return false
	}
	for index := 1; index < len(name); index++ {
		if !labelCharacter(name[index]) && (name[index] < '0' || name[index] > '9') {
			return false
		}
	}
	return true
...
func labelCharacter(character byte) bool {
