...
`Metrics.AppendMetricSamples(dst)` 把快照转换为固定 90 个稳定 sample。调用方提供
...
- Checkpoint：fence 次数、获取/持有累计耗时、最大持有耗时，以及完整 Checkpoint
  started/completed/failed、累计/最大耗时；另按 capture lock wait、capture、build、publish
  分阶段导出累计/最大耗时，并区分 capture conflict 与 Catalog publish conflict；
...
- Scheduler：requested/coalesced/completed/failed/preemptions/retries/invariant violations counter，
  queued/running gauge，以及 phase 排队等待和执行的累计/最大耗时；排队等待包含优先级排队、
  资源冲突和 phase 重新入队，不包含等待 dependency 完成的时间。Mapping survey 额外导出
  generation、physical bytes 和 reachable bytes，只有 generation 复核成功的结果才更新。
...
- 当前提供累计耗时，并为 Checkpoint fence、Checkpoint 各阶段、Scheduler queue/run、
  RecordLog rotation 和 Mapping GC 提供进程内 max；没有 histogram/quantile，p99/p999 仍应由调用路径 tracing 或外部
