...
Builder 按 `CheckpointMemoryBytes` 把 frozen layer 切成有界 ID chunk，逐 chunk 在上一轮新 Root 上继续 COW；不会再 materialize 全量 Mapping。精确 SegmentStats 从上一代表只对 folded changes 做加减；active 转动时另外顺序扫描 former-active segment 并与 candidate Mapping join。只有基线拓扑无法证明匹配时才回退全量 Root walk。SegmentStats 聚合表若超过预算会明确拒绝本次 Checkpoint 并保留 frozen layers，后续可替换为外部 run merge，不能退回无界内存。
